...
import ShapSummaryPlotsPage from '../../app/shap-summary-plots/page';
...
describe('ShapSummaryPlotsPage model switching', () => {
  beforeEach(() => {
  ...
  it('does not keep a stale error after switching models', async () => {
    ...
    fetchMock.mockImplementation(async (input, init) => {
      ...
      if (url.includes('shap-summary-plots/pharma-tablets_RF')) {
        return jsonResponse(plotPayload);
      }
      if (url.includes('shap-summary-plots/diabetes_RF')) {
    ...
    const { rerender } = render(<ShapSummaryPlotsPage />);
    ...
    const staleRequests = fetchMock.mock.calls.filter(([input, init]) => {
      const url = String(input);
      if (!url.includes('shap-summary-plots/diabetes_RF')) return false;
      const body = JSON.parse(String(init?.body));
